...
const Standings: React.FC<StandingsProps> = ({ season_id, country_id }) => {
	...
	if (error || errorTeams) {
		return <div>{error ? error : errorTeams}</div>
	}
				...
				<table>
					<thead>
						<tr>
							...
							<th scope='col'>
								<Trans t={t}>Result</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Games played</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Won</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Draw</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Lost</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Goals diff</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Goals scored</Trans>
							</th>
							<th scope='col'>
								<Trans t={t}>Goals against</Trans>
							</th>
						</tr>
					</thead>
					<tbody>
						{standingInfo.standings.map((standing, idx) => (
							<tr key={idx}>
								...
								<td data-label={`${t('Team')}`}>
									{teams.map((team) =>
										team.team_id === standing.team_id ? (
											<div
												key={team.team_id}
												className={styles.teamNameLogo}
											>
												<img
													src={team.logo}
													alt={team.name}
								...
								<td data-label={`${t('Status')}`}>
									<Trans t={t}>{standing.status}</Trans>
								</td>
								<td data-label={`${t('Result')}`}>
									<Trans t={t}>{standing.result}</Trans>
								</td>
								<td data-label={`${t('Games played')}`}>
									{standing.overall.games_played}
								</td>
								<td data-label={`${t('Won')}`}>
									{standing.overall.won}
								</td>
								<td data-label={`${t('Draw')}`}>
									{standing.overall.draw}
								</td>
								<td data-label={`${t('Lost')}`}>
									{standing.overall.lost}
								</td>
								<td data-label={`${t('Goals diff')}`}>
									{standing.overall.goals_diff}
								</td>
								<td data-label={`${t('Goals scored')}`}>
									{standing.overall.goals_scored}
								</td>
								<td data-label={`${t('Goals against')}`}>
