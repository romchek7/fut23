...
const Season: React.FC = () => {
	...
	const { season, loadingSeason, errorSeason } = useSelector(
		getSeasonByIdSelector
	)
			...
			<div className={styles.seasonBlock}>
				<div className={styles.seasonInfo}>
					<div className={styles.fixedSeasonInfo}>
						...
						<p>
							<Trans t={t}>End date</Trans>: {season?.end_date}
						</p>
					</div>
				</div>
				<div className={styles.seasonsParams}>
					{season?.season_id ? (
						<div className={styles.infoBlock1}>
							<h1>
								<Trans t={t}>Stages</Trans>:{' '}
							</h1>
							<Stages season_id={season?.season_id} />
							<div className={styles.topScorers}>
								<h1>
									<Link
										to={`/topScorers/${season.season_id}`}
									>
										<Trans t={t}>
											Top scorers of season
										</Trans>{' '}
										{season.name}
									</Link>
								</h1>
					...
					{season?.season_id ? (
						<div>
							<h1 className={styles.infoBlock1}>
								<Trans t={t}>Standings</Trans>:{' '}
							</h1>
							<Standings
					...
					{season?.season_id ? (
						<div>
							<h1 className={styles.infoBlock1}>
								<Trans t={t}>Matches</Trans>:{' '}
							</h1>
							<Matches
								season_id={season?.season_id}
								live={season?.is_current === 1}
