...
import {getMatchesActionType, IMatch, MatchesActionTypes} from "../types/matchesType";
import {getMatchesAPI} from "../../api/api";

export const fetchMatches = (season_id: number, live: boolean, status_code: number, date_from: string, date_to: string) => {
    return async (dispatch: Dispatch<MatchesActionTypes>) => {
        try {
            ...
            const response = await getMatchesAPI(season_id, live, status_code, date_from, date_to)
